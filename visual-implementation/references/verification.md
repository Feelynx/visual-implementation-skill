...
**Baseline first (existing screens).** Before editing, capture the current screen as *pixels* — render it yourself, or ask the user for current app screenshots — and run the delta table below against the source. Never reconstruct the current state from code: an extra control or a leftover gap renders but does not announce itself in source. The mismatches are the work; the post-change capture must clear every one. The same table runs at both ends — render only at the end and the user finds the defects instead of you.
