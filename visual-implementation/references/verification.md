...
**Baseline first (existing screens).** Before editing, render and capture the current screen and run the delta table below against the source; the mismatches are the work, and the post-change capture must clear every one. The same table runs at both ends — render only at the end and the user finds the defects instead of you.
...
- **Container chrome** — each top-bar / nav / FAB / bottom-bar control matches its container, not just its glyph: bare icon vs icon-in-tile (circle / pill / square), fill, shape, border, elevation. The right icon in a wrong or missing container is a *major* mismatch.
- **Presence / position / state** — the element exists, nothing extra was added, nested sub-elements are present, placement is correct, and specified states are implemented.
